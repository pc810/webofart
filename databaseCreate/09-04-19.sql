...
WHILE i<n DO 
  ...
  SET @time = (SELECT auction_end from auction WHERE auction_id = 
               (
               	SELECT auction_id from archive_auction WHERE art_id = @art_id        ));
  SET @current = (SELECT now());
  SELECT @current;
  SELECT @time;
  SET @myid = (SELECT archive_id from archive_auction LIMIT i,1);
  SET @flag = (SELECT status_art from archive_auction LIMIT i,1);
  
  IF @current > @time 
  then  
  IF @flag = "notconfirmed"
  then
  SET @myid = (SELECT archive_id from archive_auction LIMIT i,1);
  SELECT @myid;
  ...
  UPDATE archive_auction set status_art = "confirmed" WHERE archive_id = @myid;
  end IF;
  end IF;
...
--
ALTER TABLE `live_auction`
  MODIFY `live_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=29;
